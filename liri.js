...
// getTweets will retrieve last 10 tweets

function getTweets() {
    fs.appendFile('./log.txt', "User Command: node liri.js my-tweets/n/n", (err) => {
      if (err) throw err;
    });
...
// Twitter handle :
    var params = {screen_name: "_mikestank", count: 10};
      ...
      if (error) {
        ...
        '-------------------------\n\n';
          ...
          for (var i = 0; i < tweets.length; i++) {
            outputStr += "Sent: " + tweets[i].created_at + '\n' +
            'Tweet :' + tweets[i].text + '\n' +
            '---------------------\n'

            // Append to log output to file
